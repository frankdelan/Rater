...
from api.dependencies import rating_service
...
async def get_ratings_by_user(user_id: int):
    result: list[Rating] = await rating_service().list(user_id=user_id)
    if result:
...
async def get_rating_by_id(rating_id: int):
    result = await rating_service().get(id=rating_id)
    if result:
...
async def add_rating(user_id: int, title: str):
    result = await rating_service().add(user_id=user_id, title=title)
    if result:
        return ResponseSchema(data=Rating.to_schema(result))
